...
/*
    ...
    //When we are using the Query annotation you have to add the wildcard  (?) with param numbers that
    //indicates the index of the values
    //example of an update method using @Query
    //  ?1 is the wildcard, 1,2,3, are param numbers - same as the print PSTMT statements
    @Query(value = "UPDATE shops SET s_names=?1, s_inven_count?2, WHERE s_id=?3", nativeQuery = true)
